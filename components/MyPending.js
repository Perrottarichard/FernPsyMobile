...
import { initializemyPendingFiltered } from '../reducers/forumReducer';
...
const MyPending = ({navigation, route}) => {
  ...
  const DATA = myPendingFiltered?.sort((a, b) => new Date(b.date) - new Date(a.date))
  ...
  useEffect(() => {
    if(myPendingFiltered.length > 0){
      setIsLoading(false)
    }
  }, [dispatch, myPendingFiltered]);
